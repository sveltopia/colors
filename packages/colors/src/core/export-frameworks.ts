...
export function exportTailwindV4CSS(
  ...
  const allColorNames: string[] = [...sortScaleKeys(Object.keys(safePalette.light))];
  ...
  if (includeThemeBlock) {
    ...
    lines.push(' * This enables classes like bg-orange-800, text-primary-900, etc.');
    lines.push(' */');
    lines.push('@theme {');
    for (const name of allColorNames) {
      for (let step = 1; step <= 12; step++) {
        const tailwindStep = RADIX_TO_TAILWIND[step];
        lines.push(`  --color-${name}-${tailwindStep}: var(--color-${name}-${tailwindStep});`);
      }
    }
    lines.push('}');
  }
 ...
 * ```
 */
export function exportShadcn(palette: Palette, options: ShadcnExportOptions = {}): string {
  const {
    radius = '0.625rem',
    neutralHue = 'slate',
    destructiveHue = 'red',
...
export function exportRadix(palette: Palette, options: RadixExportOptions = {}): string {
  ...
  const buildScaleObject = (name: string, scale: Scale, suffix: string = ''): string => {
    const entries = [];
    for (let step = 1; step <= 12; step++) {
  ...
  const buildP3ScaleObject = (name: string, scale: Scale, suffix: string = 'P3'): string => {
    const entries = [];
    for (let step = 1; step <= 12; step++) {
      const hex = scale[step as keyof Scale];
  ...
  for (const name of sortedScaleNames) {
    ...
    if (includeAlpha) {
      ...
      if (format === 'esm') {
        ...
        exports.push(`const ${alphaName} = ${buildAlphaScaleObject(name, lightScale, 'light')};`);
      }
    }
    ...
    if (includeAlpha && includeP3) {
      ...
      if (format === 'esm') {
        ...
        exports.push(`const ${p3aName} = ${buildP3AlphaScaleObject(name, lightScale, 'light')};`);
      }
    }
    ...
    if (includeAlpha) {
      ...
      if (format === 'esm') {
        ...
        exports.push(`const ${darkAlphaName} = ${buildAlphaScaleObject(name, darkScale, 'dark')};`);
      }
    }
    ...
    if (includeAlpha && includeP3) {
      ...
      if (format === 'esm') {
        ...
        exports.push(`const ${darkP3aName} = ${buildP3AlphaScaleObject(name, darkScale, 'dark')};`);
      }
    }
