...
const TAILWIND_TO_RADIX: Record<string, number> = {
	'50': 1,
	'100': 2,
	'200': 3,
	'300': 4,
...
export function exportPanda(
	...
	// Build token structure for all scales
	const buildTokens = (scales: Record<string, Scale>): Record<string, Record<string, { value: string }>> => {
		const tokens: Record<string, Record<string, { value: string }>> = {};
		for (const [name, scale] of Object.entries(scales)) {
			tokens[name] = {};
			for (let step = 1; step <= 12; step++) {
				tokens[name][step] = { value: scale[step as keyof Scale] };
			}
		}
		return tokens;
	};

	const lightTokens = buildTokens(safePalette.light);
	const darkTokens = buildTokens(safePalette.dark);

	// Build semantic tokens if we have brand color info
	let semanticTokens = '';
	if (includeSemantic && brandColorInfo.length > 0) {
		...
		const accentSteps = [];
		for (let step = 1; step <= 12; step++) {
			accentSteps.push(`          ${step}: { value: '{colors.${primary.hue}.${step}}' }`);
		}
		accentSteps.push(`          DEFAULT: { value: '{colors.${primary.hue}.${primary.anchorStep}}' }`);
...
${accentSteps.join(',\n')}
        ...
        brand: {
          primary: { value: '{colors.${primary.hue}.${primary.anchorStep}}' },
          secondary: { value: '{colors.${secondary.hue}.${secondary.anchorStep}}' }
        }
      }
    },`;
	}

	// Build conditions for dark mode
...
export const sveltopiaColors = definePreset({
  theme: {${darkConditions}
    tokens: {
      colors: ${JSON.stringify(lightTokens, null, 8).replace(/^/gm, '      ').trim()}
    },${semanticTokens}
  }
});

// Dark mode tokens (apply via conditions or separate import)
export const sveltopiaColorsDark = {
  colors: ${JSON.stringify(darkTokens, null, 4).replace(/^/gm, '  ').trim()}
};
`;
