 ...
 */
export function meetsContrastThreshold(textHex: string, bgHex: string, threshold: number): boolean {
  ...
  if (report.issues.length > 0) {
    ...
    if (failures.length > 0) {
      ...
      for (const issue of failures) {
        lines.push(`  [FAIL] ${issue.hue} (${issue.mode}): ${issue.useCase}`);
        lines.push(
    ...
    if (warnings.length > 0) {
      ...
      for (const issue of warnings) {
        lines.push(`  [WARN] ${issue.hue} (${issue.mode}): ${issue.useCase}`);
        lines.push(
          `         Step ${issue.textStep} vs ${issue.bgStep}: Lc ${issue.actual} < ${issue.expected}`
        );
      }
    ...
    for (const [hue, scale] of Object.entries(palette[mode])) {
      ...
      if (step11 && step1) {
        ...
        if (contrast < APCA_THRESHOLDS.LARGE_TEXT) {
          newScale[11] = boostLightnessUntilContrast(step11, step1, APCA_THRESHOLDS.LARGE_TEXT);
        }
      }
      ...
      if (step12 && step1) {
        ...
        if (contrast < APCA_THRESHOLDS.BODY_TEXT) {
          newScale[12] = boostLightnessUntilContrast(step12, step1, APCA_THRESHOLDS.BODY_TEXT);
        }
      }
