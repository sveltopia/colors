  ...
  describe('Color Formatting', () => {
    ...
    it('OKLCH values are mathematically correct (round-trip)', () => {
      ...
      for (const hex of testColors) {
        ...
        expect(Math.abs(origRgb.r - reconRgb.r), `Red mismatch for ${hex}`).toBeLessThan(0.01);
        expect(Math.abs(origRgb.g - reconRgb.g), `Green mismatch for ${hex}`).toBeLessThan(0.01);
        expect(Math.abs(origRgb.b - reconRgb.b), `Blue mismatch for ${hex}`).toBeLessThan(0.01);
      }
    });
    ...
    it('P3 values are mathematically correct (round-trip)', () => {
      ...
      for (const hex of testColors) {
        ...
        // Parse the P3 string
        const match = p3Css.match(/color\(display-p3 (\d+\.?\d*) (\d+\.?\d*) (\d+\.?\d*)\)/);
