...
async function getTailwindBase(): Promise<string> {
  if (!tailwindBaseCSS) {
    const tailwindPath = join(import.meta.dirname, '../../node_modules/tailwindcss/index.css');
 ...
 */
async function compileTailwind(generatedCSS: string, candidates: string[]): Promise<string> {
...
describe('shadcn Compilation Smoke Tests', () => {
  ...
  describe('Chart tokens', () => {
    it('compiles bg-chart-1 through bg-chart-5', async () => {
      const candidates = ['bg-chart-1', 'bg-chart-2', 'bg-chart-3', 'bg-chart-4', 'bg-chart-5'];
  ...
  describe('Radius tokens from @theme inline', () => {
    ...
    it('compiles rounded-sm, rounded-md, rounded-xl with calc expressions', async () => {
      const output = await compileTailwind(shadcnCSS, ['rounded-sm', 'rounded-md', 'rounded-xl']);
