...
function loadSnapshot(brandId: string, mode: 'light' | 'dark'): Snapshot {
  const snapshotPath = join(import.meta.dirname, 'snapshots', mode, `${brandId}.json`);
  ...
  describe('Light Mode', () => {
    ...
    for (const brandId of brands) {
      it(`${brandId}: matches baseline`, () => {
        ...
        // Compare meta
        expect(current.meta.anchoredSlots.sort()).toEqual(snapshot.meta.anchoredSlots.sort());
        expect(current.meta.customSlots.sort()).toEqual(snapshot.meta.customSlots.sort());
