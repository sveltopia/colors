...
describe('CLI Generate Command E2E Tests', () => {
  ...
  describe('--colors flag', () => {
    it('generates CSS and JSON with single color', async () => {
      await execAsync(`npx tsx ${cliPath} generate --colors "#FF4F00" --output ${testDir}`, {
  ...
  describe('--format flag', () => {
    it('generates only CSS when format is css', async () => {
      ...
      const cssExists = await fs
        .access(path.join(testDir, 'colors.css'))
        .then(() => true)
        .catch(() => false);
      const jsonExists = await fs
        .access(path.join(testDir, 'colors.json'))
        .then(() => true)
        .catch(() => false);
    ...
    it('generates multiple formats with comma-separated values', async () => {
      ...
      const cssExists = await fs
        ...
        .catch(() => false);
      const tailwindExists = await fs
        .access(path.join(testDir, 'tailwind.css'))
        .then(() => true)
        .catch(() => false);
      const radixExists = await fs
        .access(path.join(testDir, 'radix-colors.js'))
        .then(() => true)
        .catch(() => false);
      const jsonExists = await fs
        .access(path.join(testDir, 'colors.json'))
    ...
    it('generates all formats when format is "all"', async () => {
      ...
      const jsonExists = await fs
        ...
        .then(() => true)
        .catch(() => false);
      const tailwindV4Exists = await fs
        .access(path.join(testDir, 'tailwind.css'))
        .then(() => true)
        .catch(() => false);
      const tailwindV3Exists = await fs
        .access(path.join(testDir, 'tailwind.preset.js'))
        .then(() => true)
        .catch(() => false);
      const radixExists = await fs
        .access(path.join(testDir, 'radix-colors.js'))
        ...
        .catch(() => false);
      const pandaExists = await fs
        .access(path.join(testDir, 'panda.preset.ts'))
        .then(() => true)
        .catch(() => false);
      const shadcnExists = await fs
        .access(path.join(testDir, 'shadcn-colors.css'))
        .then(() => true)
        .catch(() => false);
  ...
  describe('--config flag', () => {
    it('loads colors from config file', async () => {
      ...
      await execAsync(`npx tsx ${cliPath} generate --config ${configPath} --output ${testDir}`, {
        cwd: path.join(__dirname, '../..')
      });
  ...
  describe('error handling', () => {
    it('fails gracefully with invalid color', async () => {
      await expect(
        execAsync(`npx tsx ${cliPath} generate --colors "not-a-color" --output ${testDir}`, {
          cwd: path.join(__dirname, '../..')
        })
      ).rejects.toThrow();
    });
