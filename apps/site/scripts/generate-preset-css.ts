 ...
 */
function replaceHeader(css: string, preset: TestPalette, framework: string): string {
  const header = `/**
...
function main() {
  ...
  const presets = TEST_PALETTES.filter((p: TestPalette) => FEATURED_PRESET_IDS.includes(p.id));
  ...
  for (const preset of presets) {
    ...
    console.log(`  ${preset.name} -> tailwind/${preset.id}.css + shadcn/${preset.id}.css`);
  }
